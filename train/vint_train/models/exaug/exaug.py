...
class ExAug_dist_delay(BaseModel):
    def __init__(
        ...
        """
        super(ExAug_dist_delay, self).__init__(context_size, len_traj_pred, learn_angle)
    ...
    def forward(
        ...
        tokens = torch.cat((obs_encoding, goal_encoding, robot_size_repeat, delay_repeat, vel_past_repeat), dim=1)
...
class DenseNetwork_dist(nn.Module):
    def __init__(self, embedding_dim, output_dim):
        super(DenseNetwork_dist, self).__init__()
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.embedding_dim//16, output_dim)
        )
...
class DenseNetwork_action(nn.Module):
    def __init__(self, embedding_dim, output_dim):
        super(DenseNetwork_action, self).__init__()
